...
final class HttpReactor implements WorkerEntryPointInterface
{
    ...
    public function run(): void
    {
        ...
        $httpServer->start(
            new ClosureRequestHandler(static function (Request $request) use ($worker): Response {
                $result             = $worker
                                    ->getWorkerGroup()
                                    ->getJobClient()
                                    ->sendJob($request->getQueryParameter('job'), awaitResult: true)
                                    ->await();

                // Show the result of the job in the browser
                // like this:
                // Job result: Job result: Hello a job: <job>
                return new Response(
                    HttpStatus::OK,
